...
| `test_register_real.py` | **실물 발급본 3건**(건물 2·집합건물 1)에 파이프라인 전체를 고정 — 31개. 파일 없으면 skip |
| `test_market_readonly.py` | READONLY에서 외부 호출이 안 나가는지 |

**픽스처가 실제 형식과 다르면 테스트는 아무것도 보증하지 않는다.** 이 프로젝트에서 세 번
당했다 — 가짜 등기부에 '주요 등기사항 요약'이 없어서 채권최고액 2배 버그가 테스트 12개를
통과했고, 픽스처 26건이 전부 실물에 없는 '전용면적 N㎡' 줄을 써서 집합건물 추출 실패가
테스트 474개를 통과했고, 합성 픽스처의 도로명주소가 한 줄로 깔끔해서 **옆 칸 글자가 섞여 드는
실물에서 3건 중 3건이 틀린 주소를 냈다**(2026-08-02, 함정 13). 그래서
`data/fixtures/real_registers/`(미추적)가 존재한다.
...
| 8 | 공동담보 근저당을 호실 하나의 부담으로 계산 — 실측 문서는 21개 호실에 4.8억이 함께 걸려 있다 | E[Loss] 과대(보수적 방향). 경고만 있다 | 공동담보목록의 호실 수로 안분하려면 각 호실 시세가 필요하다 |
