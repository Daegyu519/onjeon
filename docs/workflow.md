...
4. **근거 등급 명시**: 모든 수치 옆에 무엇에 기대고 있는지 적는다 — 원문 대조(날짜·출처) / 2차 출처 / 판단값 / 미확보. 대조를 마치면 그 자리 문장을 출처·기준일로 바꾼다
5. **기준일 버전 관리**: 룰 DB·데이터 소스에 조회/기준일을 반드시 저장 ("2026년 ○월 기준")

## 검증 (제출 전 체크리스트)

- [x] 등기부 실물 발급본 검증 (2026-08-02: 3건 — 건물 2·집합건물 1. `tests/test_register_real.py` 31개가 고정)
- [ ] 유형 커버리지 — 스캔·촬영본과 아파트·오피스텔 실물 확보 후 유형×필드 정확도 표
...
- [ ] 모든 출력 수치에 원문 인용 표시
- [x] 문서 전체의 근거 등급 표기 정리 (2026-08-02: 마커 13건을 각 자리의 근거 문장으로 대체)
- [ ] 아직 판단값·미확보로 남은 것 — 시세 불확실성 폭, 경쟁 서비스 최신 기능(2026-07 조사),
      오피스텔·아파트 낙찰가율, 취득세·보유세·중개보수 요율 원문 대조
...
| 문제 | 대응 |
|---|---|
| L1 추출 정확도가 낮음 | (구현은 프롬프트가 아니라 텍스트 파싱이다) 실물 발급본을 `data/fixtures/real_registers/`에 넣고 `tests/test_register_real.py`에 정답을 고정한 뒤 파서를 고친다. 합성 픽스처만으로는 세 번 놓쳤다 |
| 공개 사고율 데이터 부족 | HUG 유형별 보증사고율을 베이스로 보수적 추정 + 합성 데이터로 구조 시연 (합성임을 명시) |
| 세법 조항 해석 불확실 | 확정된 것처럼 쓰지 말고 "2차 출처"라고 적어 둔 뒤, 국가법령정보 **원문 조항과 별표**를 확인해 룰 DB에 조항 출처와 함께 기록 (요율은 조문이 아니라 별표에 있다) |
