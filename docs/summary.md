...
>
> 기준: 2026-08-02 · 실측값은 배포 경로 재실행 · 테스트 573개 통과 · 리스크 모델 `risk_model_2026-08`
...
> 숫자를 만지지 않습니다.
>
> 실측해보니 혜택만 따졌을 때는 전세가 연 29만원 쌌는데, 기대손실을 반영하자 연 413만원
...
건지 알 수가 없으니까요. "연 413만원 비싸다"는 그 자체로 판단 재료가 됩니다.
...
2026년 8월 2일 배포 경로 실측값이고, 같은 입력을 넣으면 같은 답이 나옵니다.
...
기대손실이 35만원에서 4,414만원까지 벌어집니다. 125배죠. 그래서 점추정 하나를 내지 않습니다.
...
| 점추정 (P 4.19% · LGD 52.7%) | 442만원 | +413만원 |
| 밴드 상한 (P 11.63% · 시세 −30%) | 4,414만원 | +4,385만원 |
...
graph TD
    ...
    L3["L3 계산 엔진 — 결정론<br/>세후 비용 · E-Loss · 등기부 등급<br/>테스트 573개로 고정"] --> OUT
...
테스트 573개가 통과합니다. 구간표 세율, 단위 경계(만원과 원), 컨테이너 의존성, 등기부 파싱
...
| E[Loss]·비용 비교 실측치 | 배포 경로 실측(2026-08-02, `risk_model_2026-08`), 재현 가능 |
