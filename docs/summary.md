...
graph TD
    ...
    L3["L3 계산 엔진 — 결정론<br/>세후 비용 · E-Loss · 등기부 등급<br/>테스트 555개로 고정"] --> OUT
...
테스트 555개가 통과합니다. 구간표 세율, 단위 경계(만원과 원), 컨테이너 의존성, 등기부 파싱
...
형식과 다르면 테스트가 아무것도 보증하지 못한다는 걸 이미 두 번 겪었기 때문입니다.
...
5. 현재 범위는 서울 25개 구이고, 상품 룰은 7종입니다. 청년 전세·월세지원·청약, 중기청, 신혼,
   디딤돌, KB 자체 상품.
6. 가장 시급한 건 파싱 정확도 측정입니다. 실물 등기부에서 채권최고액과 전용면적의 필드별
   정확도가 아직 미측정입니다. 파싱이 틀리면 그 위의 모든 숫자가 틀립니다.
