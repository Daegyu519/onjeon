# 상세 설계도(원안) — 모듈 스펙 · 수식 · 데이터 스키마 · 화면

> 사분면: Reference. "정확히 무엇을 어떻게 만들 계획이었는가"에 답한다.
> ⚠️ **이 문서는 2026-07-19 시점의 계획이다.** 구현이 갈라진 곳이 있다 —
> L1은 비전 LLM이 아니라 텍스트 파싱, L2는 학습이 아니라 공개통계 보정,
> UI는 Streamlit이 아니라 React다. **지금 돌아가는 것**은
> [architecture.md](architecture.md)(as-built)를 보고, 갈라진 이유는 그 문서 §1 표에 있다.
> 여기 남겨두는 이유: 수식·스키마·경쟁 비교·한계 서술은 여전히 유효하고, 계획과 현물의
> 차이 자체가 설계 근거이기 때문이다.
> ⚠️ 수치의 근거는 마커가 아니라 **그 자리의 문장**으로 적는다 — 언제 무엇으로 확인했는지,
> 아니면 판단값·미확보인지. 근거 없는 숫자를 확정된 것처럼 두지 않기 위해서다.
...
3. 시세(국토부 실거래가 API — 실키 검증 2026-07-19: 관악구 2026-06 매매 154건 수신) +
   지역별 경매 낙찰가율 대입
...
2. 전세/월세/매수 3안의 **세후 총비용** 산출
   - 공통: 대출이자(KB 실측 3.49% — HF 공개 API 금액가중평균, 2026-07-28 조회),
     기회비용(연 4.0% — 청년도약계좌 기본 3.0%·우대 5.5%, 후속 청년미래적금 기본 5.0%를
     묶은 **대표값**이다. 법령 근거가 없는 판단값이라 룰 JSON에 그렇게 표시했다)
   - 월세: 세액공제 (조특법 §95-2, 총급여 구간별 15~17% — 2026-07-18 확인:
     5,500만원 이하 17% / 8,000만원 이하 15%, 한도 1,000만원)
   - 매수: 취득세·중개보수·보유세
...
P(사고): 매물 특성 기반 사고확률
  - 베이스: HUG 유형별 보증사고율 ← **구현에서 바뀌었다.** 지금은 한국부동산원 부동산테크
    시군구 공개통계(920관측·4시점)의 집계 마진 보정을 쓴다([architecture.md](architecture.md) §1)
...
위 표는 **2026-07 시점에 조사한 것이고 이후 재검색하지 않았다.** 각 서비스가 그 사이에
기능을 추가했을 수 있다. 그래서 경쟁사 칸은 후하게 적었다 — 빈 칸이 의미를 가지려면
상대를 과소평가하지 않아야 한다.
...
## 11. (로드맵) 계약서 특약 스캐너 — 모듈 C 확장

> 부메랑 등 경쟁 앱이 계약서 분석을 제공하지만 "쉬운 말 요약 + 특약 제안"의 정성 출력에서 멈춘다.
> 온전의 확장 방향: **특약의 유무를 E[Loss]에 반영**해 재무 비교에 통합한다 — 출력의 차원이 다름.

| 항목 | 내용 |
|---|---|
| 입력 | 임대차계약서 PDF → L1 확장 파싱 (특약사항 조항 텍스트 추출, `source_loc` 인용 유지) |
| 처리 | RAG 표준 특약 코퍼스(Qdrant) 대조 → ① 누락 보호특약 탐지 ② 독소조항 플래그 (판정은 룰 기반, LLM은 추출·인용만 — 원칙 1) |
| 출력 | 누락 특약 제안 + 근거 인용 + **리스크 반영 재계산** |
| 필요 데이터 | 국토부 주택임대차표준계약서 특약 문안, 법원 판례 기반 독소조항 유형 — **둘 다 아직 확보하지 못했다.** 이 모듈이 로드맵에 머무는 이유다 |

**E[Loss] 반영 수식 스케치:**

```
LGD' = LGD × (1 − Σ w_i · has_clause_i)
  예: 전세권설정 특약 → w = 0.3   (대항력·우선변제권 강화)
      ↑ 형태를 보이려고 넣은 임의값이다. 특약과 사고율의 실증 상관을 못 찾아
        실제 계수는 넣지 않았고, 그래서 이 모듈은 구현에 들어가지 않았다.
      전세보증보험 가입 조건 특약 → insured=True 경로로 승격
P(사고)' : 임대인 협조 특약(선순위 말소 조건부 등)은 P 조정 피처로 추가
```

**MVP 제외 사유**: 계약서 양식 다양성(파싱 난도), 특약 효과 계수의 실증 근거 부족 — 공개 통계 없음.
**착수 조건**: ① 표준계약서 특약 코퍼스 확보 ② L1 정확도 표 완료(등기부 파싱 신뢰 확보 후) ③ 특약-사고율 상관의 보수적 계수 합의.
