...
│   └── real_registers/  ← **실물 등기부 발급본**. 저장소 미추적(개인정보) — 있으면 tests/test_register_real.py가
│                          파이프라인 전체를 여기에 고정하고, 없으면 skip. 합성 픽스처가 놓친 것을 잡는 유일한 자리다
...
└── tests/               ← pytest 스위트 (TDD, 555 tests — 갱신 시 실제 수집 수와 맞출 것)
```
...
.venv/bin/python -m pytest    # 전체 555개 (2026-08-01 기준)
```
