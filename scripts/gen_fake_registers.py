...
def make_jiphap_pdf(path):
    """집합건물 등기부(구분소유) — 실물 형식. 위 25건은 이 형식이 **아니다**.

    위 25건은 '전용면적 70.94㎡'라는, 실물 집합건물 등기부에 없는 줄을 쓴다. 실물은
    표제부 안 '건물내역' 칸에 '철근콘크리트조 25.59㎡'로만 찍고, 같은 문서에 전용면적이
    아닌 ㎡가 세 군데 더 있다(층별 면적·대지면적·대지권비율). 이게 없는 픽스처로만
    테스트하면 파서가 대지면적을 전용면적이라 부르는 버그를 못 잡는다 — 시세 10배
    과대 → LGD 과소 → E[Loss] 과소평가(위험한 집이 안전해 보이는 방향).
    """
    lines = [
        ...
        "1  서울특별시 관악구 봉천동 100-1  대  250.30㎡  2005년5월6일 등기",  # ← 대지면적
        "【 표 제 부 】 ( 전유부분의 건물의 표시 )",
        "표시번호  접 수  건물번호  건물내역  등기원인 및 기타사항",
        "1  2005년5월6일  제3층 제301호  철근콘크리트조 25.59㎡",  # ← 이것만이 전용면적
        "( 대지권의 표시 )",
        "표시번호  대지권종류  대지권비율  등기원인 및 기타사항",
        "1  1 소유권대지권  250.30분의 21.45  2005년5월6일 대지권",  # ← 지분, 면적 아님
        "【 갑 구 】 ( 소유권에 관한 사항 )",
        "1  소유권보존  소유자 김서연  서울특별시 관악구 봉천동 100-1",
        "【 을 구 】 ( 소유권 이외의 권리에 관한 사항 )",
...
def main():
    ...
    make_jiphap_pdf(f"{out}/서울-관악구-집합건물-전유부분.pdf")
    print(f"생성 완료: {len(GU_DONG) + 2}개 → {out}/")
